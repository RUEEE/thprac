...
#define globals (GAME_MANAGER->globals)
    ...
    public:
        ...
        PATCH_HK(0x421447, "909090909090")
        HOTKEY_ENDDEF();
    };
    ...
    protected:
        ...
        virtual void OnPreUpdate() override
        {
            DWORD gameState = SUPERVISOR->gamemode;
            if (gameState == 2)
            {
                GameUpdateInner(7);
            } else {
    ...
    EHOOK_DY(th07_midi_pause, 0x44CA0C, 3, {
        if (g_adv_igi_options.th06_pauseBGM && *(BYTE*)(0x575a87) == 2) { // 0x575a87: bgm type
            ...
            if (el_switch) {
                ...
                int cmd = *(int*)(pCtx->Ebp - 0x4C);
                if (cmd == 5) // stop
                {
                    asm_call<0x436380, Thiscall>(*(DWORD*)0x575ACC); // StopTimer
                    HMIDIOUT hmid = *(HMIDIOUT*)(*(DWORD*)(0x575ACC) + 0x13C);
                    for (int i = 0; i < 16; i++) {
                ...
                {
                    asm_call<0x436300, Thiscall>(*(DWORD*)0x575ACC, 1, 0, 0); // find from midiOutOpen
                }
            }
        }
    })
    ...
    EHOOK_DY(th07_player_state, 0x441FB0, 1, {
    ...
    EHOOK_DY(th07_enter_game, 0x42EB08, 3 ,{ // set inner misscount to 0
    ...
    EHOOK_DY(th07_lock_timer2, 0x414FE6,2,{ // set timeout case 114
        g_lock_timer = 0;
    })
    EHOOK_DY(th07_lock_timer3, 0x413D06, 2, { // set boss mode case 99
    ...
    EHOOK_DY(th07_checksum1, 0x43A655,3 ,{// checksum read fix
        *(DWORD*)(pCtx->Ebp + 0xC) = *(DWORD*)(pCtx->Ebp - 0x14);
        *(DWORD*)(pCtx->Ebp + 0x10) = *(DWORD*)(pCtx->Ebp - 0x8);
    })
    EHOOK_DY(th07_checksum2, 0x435FBB,2,{ // checksum
